...
>
> 本文记录边界和迁移注意事项；第 2–6 节及第 8 节验收项已落实为当前服务端、
> 数据库迁移和导入器实现。
...
1. 里程碑通过项目 JOIN 注入用户归属，所有查询都受当前用户和未删除父项目约束。
2. 项目 DTO、创建/更新/快照均暴露 `description` 和 `color`。
3. 项目摘要接口提供进度、完成数量和下一行动读取投影。
4. 里程碑校验、游标分页、并发冲突和删除墓碑均有稳定实现。
5. 项目删除在同一事务中处理项目、子里程碑及所有墓碑事件。
6. 方寸导入器和 `fangcun_id_map` 支持稳定 ID 转换及可重复执行。
...
## 9. 与 Orialis 非语音协议路线图的关系

里程碑是 `project_milestone` 同步实体，不是 Agent command、cron 或 artifact。它
继续遵循当前 HTTP 增量事件的 `entityVersion` / `tombstone` 规则；未来 v0.3+ Agent
事件的 `event_id` / `seq` 仍是另一条事件流，不能拿来替代里程碑版本或用户
同步 cursor。

| 路线版本 | 里程碑相关约束 | 当前状态 |
| --- | --- | --- |
| v0.3–v0.4 | 里程碑 upsert/delete 保持现有 `entityType`、`entityId`、`mutationId` 字段；扩展事件只做通知 | HTTP/sync 已实现 |
| v0.5–v0.8 | clarify、approval、session、command 不改变里程碑 CRUD 或项目删除事务 | Agent 交互已实现；里程碑仍只通过既有 CRUD 写入 |
| v0.9 | cron 不从里程碑 `due` 自动生成调度；`due` 仍只是日期 | 未实现 |
| v0.10 | Voice 不进入里程碑、同步或附件协议 | 未实现且有意排除 |
| v0.11–v1.x | artifact 只能引用已授权的 Orialis 附件；不得把里程碑正文当作文件产物 | 附件与通用 artifact 事件已实现 |

因此，任何后续 Agent UI 对里程碑的操作仍应先走项目嵌套路由和 `baseVersion`，
成功后再由 sync cursor 传播；Agent 事件只可作为进度提示或请求关联。
