...
selection lenses and transition timing belong in `packages/lumina_ui/lib/`.
`mobile/lib/app/design/` contains compatibility exports and host adapters only.
...
- `LuminaSlidingSelection`: paints above labels, ignores hit testing and uses
  ...
  Navigation commits a long-press drag only on release. The outer frame and
